...
class iTransformerMinus(Module):
    @beartype
    def __init__(
        self,
        *,
    ...
    def forward(
        ...
        # Treating variates as the spatial dimension in attention
        ...
        # attention and feedforward layers with minusformer-style residual subtraction
        for attn, attn_post_norm, ff, ff_post_norm, gate in self.layers:
            residual = x
            x = attn(x) - residual  # Residual subtraction
            x = attn_post_norm(x)
            x = gate(x) * x  

            residual = x
            x = ff(x) - residual  # Residual subtraction for feedforward layer
            x = ff_post_norm(x)
            x = gate(x) * x 
        ...
        if exists(self.reversible_instance_norm):
            x = rearrange(x, 'b v n -> b n v')  # Ensure batch dimension is properly restored
            x = reverse_fn(x)
