...
class Backbone(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = iTransformerMinus(
            ...
            depth = 2,                                                
            heads = 8,                                               
            dim_head = self.hidden_dim // 8,                          
            pred_length = self.num_future_steps,                    
            num_tokens_per_variate = 1,                               
            use_reversible_instance_norm = False                   
        )
        self.Linear_layer2 = nn.Linear(3*self.hidden_dim, 2*self.hidden_dim)
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> torch.Tensor:
        ...
        t2t_valid_mask_2 = t2t_valid_mask_1.clone()                                               # [(N1,...,Nb)*H]
        t2t_valid_mask = t2t_valid_mask_1.unsqueeze(1) & t2t_valid_mask_2.unsqueeze(0)          # [(N1,...,Nb)*H, (N1,...,Nb)*H]
        ...
        m2m_h_edge_index = m2m_h_edge_index[:, m2m_h_edge_index[1] - m2m_h_edge_index[0] <= self.pos_duration]
        ...
        m_embs = m_embs.reshape(num_all_agent, self.num_historical_steps, self.num_modes*self.hidden_dim) 
